import { ClassProvider, numberAttribute } from '@angular/core';
...
export class ResizedEventPlugin extends EventManagerPlugin {
  ...
  override addEventListener(element: HTMLElement, eventName: string, handler: () => void) {
    ...
    mods.includes(DEBOUNCED_MOD) && (handler = debounced(handler, numberAttribute(mods[mods.indexOf(DEBOUNCED_MOD) + 1], undefined)));
